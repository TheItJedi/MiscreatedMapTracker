...
namespace ServerChecker
{
    ...
    class SteamServerHelper
    {
       
        public class A2S_PLAYER
        {
            // \xFF\xFF\xFF\xFFU-1\x00 because UTF-8 doesn't like to encode 0xFF
            public static readonly byte[] REQUESTCHAL = new byte[] { 0xFF, 0xFF, 0xFF, 0xFF, 0x55, 0xFF, 0xFF, 0xFF, 0xFF };
            public static readonly byte[] REQUEST = new byte[] { 0xFF, 0xFF, 0xFF, 0xFF, 0x55 };
            public byte Header { get; set; }        // I
            public A2S_PLAYER(IPEndPoint ep)
            {
                UdpClient udp = new UdpClient();
                udp.Send(REQUESTCHAL, REQUESTCHAL.Length, ep);
                ...
                byte[] allData = br.ReadAllBytes();
                ms.Seek(4, SeekOrigin.Begin);   // skip the 4 0xFFs
                Header = br.ReadByte();
                var chalNum = br.ReadInt16();
                allData[4] = 0x55;
                br.Close();
                ms.Close();
                udp.Close();
                udp = new UdpClient();
                udp.Send(allData, allData.Length, ep);
                ...
                for(int i = 1; i<= numPlayers;i++)
                {
                    ...
                    var name = ReadNullTerminatedString(ref br);
                    var score = br.ReadUInt64();
                    var duration = br.ReadDecimal();
                }
                br.Close();
                ms.Close();
                udp.Close();
